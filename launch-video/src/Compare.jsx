...
// Two native players and a sheet of type, each its own component so the scene
// can place them in its stack: the input first (under), the edit next (over
// it and over the composing shot it fades in on), the titles last. The
// edit's box is the collapse-and-slide rect the timeline writes; its
// crossfade is its own keyframes, engine-side. The input never fades on
// screen: it is lit while fully covered, and revealed by the slide.

const LABELS = [ART_LABEL_L, ART_LABEL_R];
const LIKE = FOOT[LIKE_I];

export function ComparePlayers(props) {
  const v = () => props.v;
  ...
  return (
    <>
      {/* the input: written first so it stacks under the edit, unseen
      ...
      <video
        ...
        end={T_FADED / 1000} id="lp9hli" clipHeight={28}
      >
        <keyframeTrack property="opacity" id="ne591i">
          <keyframe time={0} value={0} id="dqkjfg" />
          <keyframe time={(T_LEFT_ON - T_LEFT_IN) / 1000} value={0} id="8xkza0" />
          <keyframe time={(T_LEFT_ON - T_LEFT_IN) / 1000 + 0.034} value={1} id="wynowq" />
          <keyframe time={(T_FADE - T_LEFT_IN) / 1000} value={1} easing={FADE_EASE} id="mte61r" />
          <keyframe time={(T_FADED - T_LEFT_IN) / 1000} value={0} id="gigs2b" />
        </keyframeTrack>
        ...
        <shaderPaint
          wgsl={LOG_WGSL}
          uniforms={{ sat: LOG_SAT, contrast: LOG_CONTRAST }} id="8chxtl"
        />
      </video>
      ...
      <video
        ...
        muted id="kosm4a" clipHeight={28}
      >
        <keyframeTrack property="opacity" id="hxzjyv">
          <keyframe time={0} value={0} easing={FADE_EASE} id="j8bvos" />
          <keyframe time={ARRIVE / 1000} value={1} id="60qwne" />
          <keyframe time={(T_FADE - T_PLAY) / 1000} value={1} easing={FADE_EASE} id="h5nxyt" />
          <keyframe time={(T_FADED - T_PLAY) / 1000} value={0} id="sajd0k" />
        </keyframeTrack>
      </video>
    </>
  );
...
export function CompareTitles(props) {
  const v = () => props.v;
  ...
  return (
    <>
      ...
      <rect
        ...
        id="1k0hmb" clipHeight={28}
      >
        <keyframeTrack property="opacity" id="yxjjzq">
          <keyframe time={(T_FADE - T_SET1) / 1000} value={1} easing={FADE_EASE} id="t9fsd1" />
          <keyframe time={(T_FADED - T_SET1) / 1000} value={0} id="o8y37v" />
        </keyframeTrack>
        <htmlPaint id="6o9gl5">
